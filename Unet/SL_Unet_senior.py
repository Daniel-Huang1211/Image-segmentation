...
class SupervisedSegmentation:
    def __init__(self,           #將以下條件初始化
        ...
        """
        在下面這段程式碼中，self.apex 是一個布林值，它控制是否使用 NVIDIA 的 Apex 套件來進行混合精度訓練。
        混合精度訓練是一種訓練神經網路的技術，通過將部分運算轉換為低精度（通常是半精度浮點數）來節省記憶體和計算資源，從而提高訓練速度。
        如果 self.apex 為 True，則程式會執行以下操作：
        1.使用 amp.initialize 函式初始化模型和優化器。該函式會將模型和優化器轉換為支持混合精度訓練的版本。這樣，在後續的訓練過程中，模型的部分運算將以低精度執行。
        2.opt_level="O1" 參數指定了混合精度訓練的最佳化級別。"O1" 級別是 Apex 中的一個選項，它啟用了一些基本的混合精度訓練功能，同時保持了模型的訓練穩定性。
        這段程式碼的作用是在程式開始訓練前，根據 self.apex 的值來決定是否啟用混合精度訓練，從而提高訓練效率。
        """
        self.apex = apex
...
if __name__ == "__main__":
    ...
    parser.add_argument("--v_txt_path", "--valid_txt_path", default="/home/jannawu/Desktop/all_cag_files/400/single/4aug/valid/txt/valid_2.txt")
